...
static int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub,
		int argc, char *argv[]);
...
void print_client_sub_conf(struct mosq_config *cfg)
{
	/*
		char *id;
		char *id_prefix;
		...
		int msg_count; //sub
	*/
	printf("The id is: %s", cfg->id);
	printf("The idprefix is: %s", cfg->id_prefix);
	printf("The protocol_version is: %d", cfg->protocol_version);
	printf("The keepalive is: %d", cfg->keepalive);
	printf("The host is: %s", cfg->host);
	printf("The port is: %d", cfg->port);
	printf("The qos is: %d", cfg->qos);
	printf("The retain is: %d", cfg->retain);
	//The below are valid for only the publisher
	//printf("The pub_mode is: %d", cfg->pub_mode);// pub
	//printf("The file_input is: %s", cfg->file_input);// pub
	//printf("The file_input is: %s", cfg->message);// pub
	//printf("The file_input is: %d", cfg->msglen);// pub
	//printf("The file_input is: %s", cfg->topic);// pub

	printf("The file_input is: %s", cfg->bind_address);
	printf("The file_input is: %d", cfg->debug);

	printf("The file_input is: %d", cfg->quiet);
	printf("The file_input is: %d", cfg->max_inflight);
	printf("The file_input is: %s", cfg->username);
	printf("The file_input is: %s", cfg->password);
	printf("The file_input is: %s", cfg->will_topic);
	printf("The file_input is: %s", cfg->will_payload);
	printf("The file_input is: %d", cfg->will_payloadlen);
	printf("The file_input is: %d", cfg->will_qos);
	printf("The file_input is: %d", cfg->will_retain);
	printf("The file_input is: %d", cfg->clean_session);//sub
	//printf("The file_input is: %s", cfg->topics); //sub
	printf("The file_input is: %d", cfg->topic_count); //sub
	printf("The file_input is: %d", cfg->no_retain); //sub
	//printf("The file_input is: %s", cfg->filter_outs);//sub
	printf("The file_input is: %d", cfg->filter_out_count); //sub
	printf("The file_input is: %d", cfg->verbose); //sub
	printf("The file_input is: %d", cfg->eol); //sub
//	printf("The file_input is: %d", cfg->msg_count); //sub
...
int client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc,
		char *argv[])
{
	int rc;
	FILE *fptr;
	...
	printf("The env variable is: %s \n", env);
	if (env)
	{
		printf("I am inside env=1 \n");
	...
	else
	{
		...
		if (env)
		{
			...
			if (pub_or_sub == CLIENT_PUB)
			{
				...
				printf("%s/mosquitto_pub \n", env);
			}
			else
			{
				snprintf(loc, len, "%s/.config/mosquitto_sub", env);
				printf("%s/mosquitto_sub \n", env);
			}
			loc[len - 1] = '\0';
		}
		else
...
int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc,
		char *argv[])
{
	int i;
	...
	for (i = 1; i < argc; i++)
	{
		...
		else if (!strcmp(argv[i], "-t") || !strcmp(argv[i], "--topic"))
		{
			...
			else
			{
				if (pub_or_sub == CLIENT_PUB)
				{
					if (mosquitto_pub_topic_check(argv[i + 1])
							== MOSQ_ERR_INVAL)
					{
						fprintf(stderr,
				...
				{
					if (mosquitto_sub_topic_check(argv[i + 1])
							== MOSQ_ERR_INVAL)
					{
						fprintf(stderr,
		...
		else if (!strcmp(argv[i], "-T") || !strcmp(argv[i], "--filter-out"))
		{
			...
			else
			{
				...
				cfg->filter_outs[cfg->filter_out_count - 1] = strdup(
						argv[i + 1]);
			}
			i++;
#ifdef WITH_TLS
		...
		else if (!strcmp(argv[i], "--will-topic"))
		{
			...
			i++;
				...
				|| !strcmp(argv[i], "--disable-clean-session"))
		{
			if (pub_or_sub == CLIENT_PUB)
			{
				goto unknown_option;
...
//The below function applies for both the subscriber and publisher client.
int client_id_generate(struct mosq_config *cfg, const char *id_base)
{

	int len;
	char hostname[256];
	print_client_sub_conf(cfg);
	if (cfg->id_prefix)
	{
		printf("Naming the client: %s : \n", cfg->id_prefix);
