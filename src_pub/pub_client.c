...
#define TIMESTAMP $(shell date "+%F %T%z")
#define SOVERSION 1
#define XSLTPROC xsltproc
#define DB_HTML_XSL man/html.xsl
#define UNAME $(shell uname -s)
...
{
	printf("I am inside the my_connect_call_back \n");
	...
	if (!result)
	{
		...
		case MSGMODE_STDIN_FILE:
			rc = mosquitto_publish(mosq, &mid_sent, topic, msglen, message, qos,
					retain);
			break;
		case MSGMODE_NULL:
			rc = mosquitto_publish(mosq, &mid_sent, topic, 0, NULL, qos,
					retain);
			break;
...
{
	printf("i am in the my_disconnect_callback function \n");
	connected = false;
}

void my_publish_callback(struct mosquitto *mosq, void *obj, int mid)
{
	printf("i am in the my_publish_callback function call \n");
	last_mid_sent = mid;
...
void my_log_callback(struct mosquitto *mosq, void *obj, int level,
		const char *str)
{
	printf("%s I am in the  in my_log_callback \n", str);
}

int load_stdin(void)
{
	printf("I am in the load_stdin function \n");
	long pos = 0, rlen;
	char buf[1024];
...
int main(int argc, char *argv[])
{
	...
	printf("Setting up memory : %d \n ", sizeof(struct mosq_config));
	rc = client_config_load(&cfg, CLIENT_PUB, argc, argv);

	printf("Loading the client with configuration details. rc value is: %d \n ", rc);
	if (rc)
	{
	...
	quiet = cfg.quiet;
	printf("the loaded values are as follows topic1=%s \n , message= %s  \n ",
				topic, message);
	...
	printf("mode is : %d  and status is %d \n", mode, status );
	do
	{
		if (mode == MSGMODE_STDIN_LINE)
		{
			if (status == STATUS_CONNACK_RECVD)
			{
				...
				while (fgets(&buf[pos], read_len, stdin))
				{
					...
					if (buf[buf_len_actual - 1] == '\n')
					{
						...
						rc2 = mosquitto_publish(mosq, &mid_sent, topic,
								buf_len_actual, buf, qos, retain);
						if (rc2)
						{
							if (!quiet)
								fprintf(stderr,
		...
		{
			printf("else part of the do while loop \n");
